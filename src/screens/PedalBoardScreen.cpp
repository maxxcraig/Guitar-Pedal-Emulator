    ...
    : onBackToHome(goHome)
{
    ...
    backgroundImage = juce::ImageCache::getFromFile(
        juce::File::getCurrentWorkingDirectory().getChildFile("resources/Pedals.png"));
    ...
    delayPedal       = std::make_unique<DelayComponent>();
    tremoloPedal     = std::make_unique<TremoloComponent>();
    phaserPedal      = std::make_unique<PhaserComponent>();
